...
export default function Login() {
  ...
  const { login, loginWithGoogle, resetPassword, user, isMocked, resendVerificationEmail, confirmMockEmail } = useAuth();
  ...
  // Verification Screen States (in case login returns email not confirmed)
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    setIsLoading(true);
    const { error: loginError, needsVerification: shouldVerify } = await login(formData.email, formData.password);
  ...
  return (
    <main className="min-h-screen flex items-center justify-center bg-[#0B0F19] p-4 relative overflow-hidden">
      <div className="absolute top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 w-full max-w-md h-[400px] bg-primary/20 blur-[120px] -z-10 rounded-full"></div>
        ...
        {needsVerification ? (
          ...
          <div className="text-center animate-fade-in">
            ...
            <p className="text-gray-400 text-sm mb-6 leading-relaxed">
              Sua conta ainda não foi confirmada. <br />
            ...
            <div className="space-y-4">
              ...
              <div className="pt-2">
                <button
                  onClick={() => { setNeedsVerification(false); setError(""); setSuccessMessage(""); }}
          ...
          <>
            ...
            <div className="text-center mb-8">
              ...
              <p className="text-gray-400 mt-2">
                {isRecovering 
                  ...
                  : "Acesse sua conta para continuar criando"}
              </p>
            </div>

            {/* Dynamic Alerts */}
            {error && (
              <div className="mb-5 bg-red-500/10 border border-red-500/50 text-red-400 text-sm p-3 rounded-lg flex items-center gap-2 animate-fade-in">
            ...
            {successMessage && (
              <div className="mb-5 bg-emerald-500/10 border border-emerald-500/50 text-emerald-400 text-sm p-3 rounded-lg flex items-center gap-2 animate-fade-in">
                <svg className="w-4.5 h-4.5 shrink-0" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                <span>{successMessage}</span>
              </div>
            )}

            {/* 1. PASSWORD RECOVERY FORM */}
            {isRecovering ? (
              <form onSubmit={handlePasswordRecovery} className="space-y-5">
                <div>
                  <label className="block text-sm font-medium text-gray-300 mb-1.5">Seu E-mail</label>
                  <input 
                    type="email" 
                    value={recoveryEmail}
                ...
                <button 
                  type="submit"
                  disabled={isLoading}
                  className="w-full bg-gradient-to-r from-primary to-secondary text-white font-semibold rounded-lg px-4 py-3 hover:shadow-[0_0_20px_rgba(139,92,246,0.3)] transition-all transform hover:-translate-y-0.5 disabled:opacity-70 disabled:hover:translate-y-0 flex justify-center items-center gap-2"
                >
                  {isLoading ? (
                    <>
              ...
              <>
                ...
                <div className="space-y-4 mt-6">
                  ...
                  <button 
                    ...
                    disabled={isLoading || isGoogleLoading}
                    className="w-full bg-[#1A1F2C] border border-white/10 hover:bg-[#242B3C] text-white font-medium rounded-lg px-4 py-3 transition-colors flex items-center justify-center gap-3 disabled:opacity-60 shadow-lg cursor-pointer text-sm"
                  >
                    {isGoogleLoading ? (
                      <svg className="w-5 h-5 animate-spin text-white" fill="none" viewBox="0 0 24 24">
                        <circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle>
            ...
            <div className="mt-8 text-center text-sm text-gray-400">
              Não tem uma conta? <Link href="/register" className="text-primary hover:text-white transition-colors font-medium">Cadastre-se grátis</Link>
            </div>
          </>
        )}
